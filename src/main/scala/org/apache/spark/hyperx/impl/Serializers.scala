...
//                writeUnsignedVarInt(msg._1)
        ...
        override def deserializeStream(s: InputStream) = new
            ...
            override def readObject[T: ClassTag](): T = {
//                (readUnsignedVarInt(), null).asInstanceOf[T]
                (readVarLong(optimizePositive = true), null).asInstanceOf[T]
            }
        }
    }
}
...
private[hyperx]
abstract class ShuffleSerializationStream(s: OutputStream) extends
SerializationStream {
    ...
    def writeDouble(v: Double): Unit = writeLong(java.lang.Double
            .doubleToLongBits(v))
...
private[hyperx]
abstract class ShuffleDeserializationStream(s: InputStream) extends
DeserializationStream {
    ...
    def readInt(): Int = {
        ...
        (first & 0xFF) << 24 | (s.read() & 0xFF) << 16 | (s.read() & 0xFF) <<
                8 | (s.read() & 0xFF)
    }
    ...
    def readUnsignedVarInt(): Int = {
        ...
        while ((b & 0x80) != 0) {
            value |= (b & 0x7F) << i
            i += 7
            if (i > 35) throw new IllegalArgumentException("Variable length " +
...
private[hyperx] sealed trait ShuffleSerializerInstance extends SerializerInstance {
