 ...
 * implicit evidence of membership in the `VertexPartitionBaseOpsConstructor`
 * typeclass (for
 * example, [[VertexPartition.VertexPartitionOpsConstructor]]).
 *
 * Forked from GraphX 2.10, modified by Jin Huang
 */
private[hyperx] abstract class VertexPartitionBaseOps
[VD: ClassTag, Self[X] <: VertexPartitionBase[X] :
...
(self: Self[VD])
    ...
    def diff(other: Self[VD]): Self[VD] = {
        if (self.index != other.index) {
            logWarning("Diffing two VertexPartitions with different indexes " +
                    "is slow.")
    ...
    (f: (VertexId, VD, Option[VD2]) => VD3): Self[VD3] = {
        if (self.index != other.index) {
            logWarning("Joining two VertexPartitions with different indexes " +
                    "is slow.")
            ...
            var i = self.mask.nextSetBit(0)
            while (i >= 0) {
                val otherV: Option[VD2] = if (other.mask.get(i)) Some(other
                        .values(i)) else None
    ...
    /**
     * Similar to innerJoin, but vertices from the left side that don't
     * appear in iter will remain in the partition, hidden by the bitmask.
     */
...
//        System.arraycopy(self.values, 0, newValues, 0, newValues.length)
    ...
    def outerJoin(iter: Iterator[Product2[VertexId, VD]]): Self[VD] = {
        val newMask = new BitSet(self.capacity)
        val newValues = new Array[VD](self.capacity)
        System.arraycopy(self.values, 0, newValues, 0, newValues.length)
        iter.foreach { pair =>
    ...
    /**
     * Construct a new VertexPartition whose index contains only the vertices
     * in the mask.
     */
    def reindex(): Self[VD] = {
        val hashMap = new HyperXOpenHashMap[VertexId, VD]
