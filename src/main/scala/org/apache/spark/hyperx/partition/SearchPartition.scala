...
abstract class SearchPartition extends HeuristicPartition {
    ...
    :(RDD[(PartitionId, VertexId)], RDD[(PartitionId, String)])= {
        ...
        logInfo("HYPERX DEBUGGING: P0 partition.collect in %d ms"
        ...
        logInfo("HYPERX DEBUGGING: P1 unassign in %d ms"
                .format(System.currentTimeMillis() - start))
        start = System.currentTimeMillis()
        search()
        logInfo("HYPERX DEBUGGING: P2 search in %d ms"
                .format(System.currentTimeMillis() - start))
        printFinalMsg(start)
