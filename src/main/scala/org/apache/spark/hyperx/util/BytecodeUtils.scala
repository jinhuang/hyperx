...
/**
 * Includes an utility function to test whether a function accesses a
 * specific attribute
 * of an object.
 *
 * Forked from GraphX 2.10, modified by Jin Huang
 */
private[hyperx] object BytecodeUtils {

    /**
     * Test whether the given closure invokes the specified method in the
     * specified class.
     */
    def invokedMethod(closure: AnyRef, targetClass: Class[_],
                      targetMethod: String): Boolean = {
        if (_invokedMethod(closure.getClass, "apply", targetClass,
            targetMethod)) {
            true
        } else {
    ...
    private def _invokedMethod(cls: Class[_], method: String,
                               targetClass: Class[_], targetMethod: String):
    Boolean = {

        val seen = new mutable.HashSet[(Class[_], String)]
        ...
        while (stack.nonEmpty) {
            ...
            for (classMethod <- finder.methodsInvoked) {
                // println(classMethod)
                if (classMethod._1 == targetClass && classMethod._2 ==
                        targetMethod) {
                    return true
    ...
    private def getClassReader(cls: Class[_]): ClassReader = {
        // Copy data over, before delegating to ClassReader - else we can run
    ...
    /**
     * Given the class name, return whether we should look into the class or
     * not. This is used to
     * skip examing a large quantity of Java or Scala classes that we know
     * for sure wouldn't access
     * the closures. Note that the class name is expected in ASM style (i.e.
     * use "/" instead of ".").
     */
    ...
    /**
     * Find the set of methods invoked by the specified method in the
     * specified class.
     * For example, after running the visitor,
     * MethodInvocationFinder("spark/graph/Foo", "test")
     * its methodsInvoked variable will contain the set of methods invoked
     * directly by
     * Foo.test(). Interface invocations are not returned as part of the
     * result set because we cannot
    ...
    private class MethodInvocationFinder(className: String, methodName: String)
        ...
        val methodsInvoked = new mutable.HashSet[(Class[_], String)]
